...
bank_list  = ["Bank A", "Bank B", "Bank C"]
...
def refresh_table():
        ...
        with container:
            with ui.row().classes('items-center gap-2 p-2 rounded-xl shadow-md  w-full').style(f'background-color: {TABLE_ADD_BG};'):
                if entry["editable"]:
                    ...
                    ui.button(icon='save', on_click=lambda i=index, b=bank_input, a=account_input, p=password_input: save_row(i, b, a, p), color=BUTTON_COLOR).classes('w-10')
                else:
                    ui.label(entry["bank_name"]).classes('w-32')
                    ui.label(entry["account"]).classes('w-32')
                    ui.label("••••••••").classes('w-32')
                    ui.button(icon='edit', on_click=lambda i=index: edit_row(i), color=BUTTON_COLOR).classes('w-10')

                # Delete icon (except new-entry row)
                if index < len(accounts):
                    ui.button(icon='delete', on_click=lambda i=index: delete_row(i), color=BUTTON_COLOR).classes('w-10')
    ...
    with container:
        with ui.row().classes('items-center gap-2 p-2 rounded-xl shadow-md w-full').style(f'background-color: {TABLE_BG};'):
            new_bank = ui.select(options=bank_list, label="Bank").classes('w-32').props(f'label-color=grey-5')
            ...
            ui.button(icon='add', on_click=lambda: add_row(new_bank, new_account, new_password), color=BUTTON_COLOR).classes('w-10')

def edit_row(index):
    accounts[index]["editable"] = True
    refresh_table()

def save_row(index, bank_input, account_input, password_input):
    accounts[index] = {
...
def add_row(bank_input, account_input, password_input):
...
def start_ui():
    ...
    with ui.row().classes('w-full justify-center'):
        with ui.column().classes('items-center') as container:
            pass
